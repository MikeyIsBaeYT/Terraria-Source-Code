...
namespace Terraria
{
  public class Lang
  {
    ...
    public static LocalizedText GetItemName(int id)
    {
      ...
      return id > 0 && id < 3930 && Lang._itemNameCache[id] != null ? Lang._itemNameCache[id] : LocalizedText.Empty;
    }
    ...
    public static string BartenderHelpText(NPC npc)
    {
      ...
      if (player.bartenderQuestLog == 0)
      {
        ...
        Item obj = player.GetItem(player.whoAmI, newItem, true);
        if (obj.stack > 0)
        {
      ...
      bool showAllAvailableUnits)
    {
      ...
      if (time.Hours > 0)
      {
        string str3 = str1 + (object) time.Hours + (abbreviated ? (object) (" " + Language.GetTextValue("Misc.ShortHours")) : (time.Hours == 1 ? (object) " hour" : (object) " hours"));
        if (!showAllAvailableUnits)
          return str3;
        str1 = str3 + " ";
      }
      if (time.Minutes > 0)
      {
        string str4 = str1 + (object) time.Minutes + (abbreviated ? (object) (" " + Language.GetTextValue("Misc.ShortMinutes")) : (time.Minutes == 1 ? (object) " minute" : (object) " minutes"));
        if (!showAllAvailableUnits)
          return str4;
        str1 = str4 + " ";
      }
